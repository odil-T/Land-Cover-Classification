...
def display(image_path):
    ...
    # Create an empty image to draw the approximated contours
    approx_mask = np.zeros_like(merged_mask)

    # Loop over each color/feature
    for color in colormap:
        # Create a binary mask for the current feature color
        feature_mask = cv2.inRange(merged_mask, np.array(color), np.array(color))
        ...
        for contour in contours:
            ...
            # Draw the approximated contour on the new mask with the original color
            cv2.drawContours(approx_mask, [approx], -1, color, thickness=cv2.FILLED)

    # Display the original mask and the approximated mask

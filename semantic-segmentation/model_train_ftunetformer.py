"""
Trains and saves an FT U-Net Former model on OpenEarthMap Dataset for semantic segmentation.
"""
...
outputs_save_dir = f"models/ftunetformer_sem_seg_{current_datetime}"
...
# Model Preparation
model = ft_unetformer(num_classes=num_classes).to(device)
...
for epoch in range(epochs):
    ...
    if epoch % 5 == 0 and epoch != 0:
        save_checkpoint(model, optimizer, epoch,
                        os.path.join(outputs_save_dir, f"ftunetformer_sem_seg_checkpoint_epoch{epoch}.pt"))
...
torch.save(model, os.path.join(outputs_save_dir, "ftunetformer_sem_seg.pth"))
